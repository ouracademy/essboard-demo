...
describe("session", () => {
  ...
  beforeAll(async done => {
    ...
    ProjectService.create(qpdian, "Ouracademy").then(data => {
      project = data;
      Promise.all(
        SessionService.create(data.key, 1, currentDate),
        MemberService.create(data.key, armadeit)
      ).then(([sessionValue, member]) => {
  ...
  describe("start new session", () => {
    it("should start a new session with current date", async () => {
      await expect(session.createdAt).toEqual(currentDate);
    });
